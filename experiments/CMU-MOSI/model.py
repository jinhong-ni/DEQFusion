...
class SmallBlockV2(nn.Module):
    def __init__(self, out_dim, deq_expand=2, num_groups=2, dropout=0.0, wnorm=False):
        ...
        """
        super(SmallBlockV2, self).__init__()

        self.out_dim = out_dim

#         self.conv1 = torch.nn.Conv1d(self.out_dim, self.out_dim, 1)
#         self.conv2 = torch.nn.Conv1d(self.out_dim, self.out_dim, 1)
#         self.gn1 = torch.nn.BatchNorm1d(self.out_dim)
#         self.gn2 = torch.nn.BatchNorm1d(self.out_dim)#nn.GroupNorm(1, inplanes, affine=True)
#         self.relu2 = nn.ReLU(inplace=True)
        self.conv1 = torch.nn.Conv1d(self.out_dim, self.out_dim, 1)
        self.gn1 = nn.GroupNorm(5, self.out_dim, affine=True)#torch.nn.BatchNorm1d(self.out_dim)
        self.relu = nn.ReLU(inplace=True)
        
        self.conv2 = torch.nn.Conv1d(self.out_dim, self.out_dim, 1)
        self.gn2 = nn.GroupNorm(5, self.out_dim, affine=True)#torch.nn.BatchNorm1d(self.out_dim)

        self.gn3 = nn.GroupNorm(5, self.out_dim, affine=True)#torch.nn.BatchNorm1d(self.out_dim)
...
class EQFusionBlockV2(nn.Module):
    def __init__(self, num_out_dims, deq_expand=2, num_groups=1, dropout=0.0, wnorm=False):
        ...
        A canonical residual block with two 1x1 convolutions and an intermediate ReLU.
        """
        super(EQFusionBlockV2, self).__init__()
...
#         self.conv_inject = torch.nn.Conv1d(self.branch1_dim + self.branch2_dim, self.out_dim, 1)
        self.gn2 = nn.GroupNorm(5, self.out_dim, affine=True)#torch.nn.LayerNorm(self.out_dim)#torch.nn.BatchNorm1d(self.out_dim)
        self.gn3 = nn.GroupNorm(5, self.out_dim, affine=True)#torch.nn.LayerNorm(self.out_dim)#torch.nn.BatchNorm1d(self.out_dim) #nn.GroupNorm(1, inplanes, affine=True)
        self.relu3 = nn.ReLU(inplace=True)
        self.conv3 = torch.nn.Conv1d(self.out_dim, self.out_dim, 1)
            
    def forward(self, x, injection_feature_branch1, injection_feature_branch2, residual_feature):
        ...
        out = extracted_fea1 + extracted_fea2
#         out = torch.tanh(self.conv3(out))
        out = self.conv3(out) + residual_feature
...
#         injection_feature = torch.cat((residual_feature_branch1, residual_feature_branch2), dim=1)
#         injection_feature = self.conv_inject(injection_feature)
        out = extracted_fea1 + extracted_fea2 + residual_feature
        out = self.relu3(self.conv3(self.gn3(out)))
...
class DEQFusionModule(nn.Module):
    def __init__(self, num_out_dims):
        super(DEQFusionModule, self).__init__()
    ...
    def forward(self, x, injection):
        ...
        for i in range(self.num_branches - 1):
            ...
            x_block_out.append(out)
        inject_fusion_feature = x_block_out[0] + x_block_out[1]
        x_block_out.append(self.branches[self.num_branches - 1](x[self.num_branches - 1], inject_fusion_feature, injection[0], injection[1]))
        return x_block_out

class BertFinetuneInn(nn.Module):
    def __init__(self, config):
        super(BertFinetuneInn, self).__init__()
        ...
        self.func_ = DEQEQFusionModule([50, 50, 50])
        self.f_thres = 105#35
        self.b_thres = 106#36
        self.stop_mode = 'abs'#"rel"
...
#         print(audio_feature.min(),audio_feature.max())
#         print(text_feature.min(),text_feature.max())
        ...
        z_list = [torch.zeros(batch_size, dim_size, dim_size).cuda() for dim_size in out_dim_list]
...
#                 print(z1[0,:,0])
    ...
    def forward(self, hidden_states,pooled_output,audio_data,attention_mask):
        ...
        audio_data = audio_data.transpose(1, 2)
#         print(audio_data.shape,text_data.shape,'!!!!!')
        ...
        fusion_feature = text_weight_1*text_att1 + audio_weight_1*audio_att + bias 
#         print(audio_att.shape)
#         print(text_att1.shape, "text att shape")
        fusion_att, jacbian_loss, trace = self.featureFusion(-1, audio_att, text_att1, fusion_feature)
        fusion_att = fusion_att[-1]
        ...
        return hidden_states_new,text_att1,fusion_att1,jacbian_loss,trace
...
#         self.BertFinetun = BertFinetun(config)
        self.BertFinetun = BertFinetuneInn(config)
...
#         trace = {}
        pooled_output,text_att,fusion_att,jacbian_loss,trace = self.BertFinetun(encoder_lastoutput,pooled_output,all_audio_data,extend_mask)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        if labels is not None:
            loss = 0.5 * (logits.view(-1) - labels) ** 2 
#             print(loss.mean(),jacbian_loss.mean())
            loss = loss + 0.01 * jacbian_loss.mean()
            return loss
        else:
            return logits,text_att,fusion_att,trace
