...
def train_epoch(data_list, label, model, optimizer):
    model.train()
    optimizer.zero_grad()
    loss, _, loss_dict = model(data_list, label)
...
def train(data_folder, modelpath, testonly):
    ...
    data_tr_list, data_test_list, trte_idx, labels_trte = prepare_trte_data(data_folder, use_view=['mrna', 'mirna'])
    labels_tr_tensor = torch.LongTensor(labels_trte[trte_idx["tr"]])
    onehot_labels_tr_tensor = one_hot_tensor(labels_tr_tensor, num_class)
    labels_tr_tensor = labels_tr_tensor.cuda()
    onehot_labels_tr_tensor = onehot_labels_tr_tensor.cuda()
    dim_list = [x.shape[1] for x in data_tr_list]
    model = MMDynamic(dim_list, hidden_dim, num_class, dropout=0.5, jacobian_weight=100, f_thres=105, b_thres=106, use_deq=True)
    model.cuda()
